...
public class LocalMUCUser implements MUCUser {
    ...
    public void process(Message packet) {
        ...
        if (group == null) {
            // Packets to the groupchat server. This should not occur (should be handled by
            // MultiUserChatServiceImpl instead)
        ...
        try {
            if (role.getChatRoom().getRoomHistory().isSubjectChangeRequest(packet)) {
                ...
                if (resource == null && Message.Type.groupchat == type) {
                    ...
                    } else if (userInfo.element("oper") != null) {
                        // TODO log oper
                        role.getChatRoom().sendPublicMessage(packet, role);
                    } else {
    ...
    public void process(IQ packet) {
        ...
        if (group == null) {
            ...
            MUCRole role = roles.get(group);
            if (role == null) {
                Log.debug("Ignoring stanza received from a non-occupant of '{}': {}", group,
                    packet.toXML());
                if (packet.isRequest()) {
    ...
    public void process(Presence packet) {
        ...
        if (group != null) {
            ...
            if (role == null || mucInfo != null) {
                // If we're not already in a room (role == null), we either are joining it or it's
                // not
                // properly addressed and we drop it silently
                // Alternative is that mucInfo is not null, in which case the client thinks it isn't
                // in the room, so we should join anyway.
                if (recipient.getResource() != null
                    ...
                    if (packet.isAvailable()) {
                        try {
                            ...
                            // the room creator provides an initial configuration and therefore
                            // before the room officially exists), the service MUST refuse entry and
                    ...
                    if (packet.isAvailable()) {
                        ...
                        // If the user does not specify a room nickname (note the bare JID on the
                        // 'from' address in the following example), the service MUST return a
                        // <jid-malformed/> error
                        sendErrorPacket(packet, PacketError.Condition.jid_malformed);
                    }
                ...
                if (!role.getUserAddress().equals(packet.getFrom())) {
                    ...
                    if (Presence.Type.unavailable == packet.getType()) {
                        try {
                            // TODO Consider that different nodes can be creating and processing
                            // this presence at the same time (when remote node went down)
                            removeRole(group);
                        ...
                        try {
                            ...
                            if (resource == null
                                ...
                                // Answer a conflic error if the new nickname is taken
                                    ...
                                    role.getChatRoom().nicknameChanged(role, packet, oldNick,
                                        resource);
                                }
                            }
                        } catch (Exception e) {
